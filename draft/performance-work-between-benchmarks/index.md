...
Tomorrow's [What Go Taught Us About Java Garbage Collection](https://www.codenameone.com/blog/parparvm-gc-small-heaps/) asks why a collector should keep memory that buys no speed, and how to stop a cache flush from making you decode every image again. On Sunday, [Faster Maps: Chasing Swiss Speed](https://www.codenameone.com/blog/hashmap-misses-probe-sequence/) follows our comparison with Go's map design into faster lookups and numbers that no longer need their own heap objects.
...
![Reported resident memory at five GC trigger settings](gc-trigger-rss-609892520e.svg)
...
The [collector and cache article](https://www.codenameone.com/blog/parparvm-gc-small-heaps/) follows both experiments in detail. Lowering garbage headroom and retaining useful cached results address different sides of the same memory budget.
...
A JSON parser filling a map may allocate a wrapper for every number. Our [earlier performance work](https://www.codenameone.com/blog/beating-hotspot-performance/) removed much of that cost for Integer. [PR #5735](https://github.com/codenameone/CodenameOne/pull/5735) extends the representation to Short, Character, Float, Long, and Double.
...
The [maps and boxing article](https://www.codenameone.com/blog/hashmap-misses-probe-sequence/) covers the coverage measurements and dispatch hazards. A type tag must select the right `hashCode` and `equals` implementation. Reusing the Integer fast path for every tag would produce a fast, incorrect map.
...
The [native and JavaScript performance article](https://www.codenameone.com/blog/startup-cost-before-first-paint/) follows these paths and their tests. The common question is whether an operation needs to wait at all, before trying to make its waiting machinery faster.
...
The migration also checked **2,272 pages and 29,583 fragments** against the old output. A prettier member page is not much use if years of links to that member stop working. The [doclet article](https://www.codenameone.com/blog/javadoc-hugo-markdown-doclet/) includes a complete sample, build commands, and the anchor defects that parity testing caught.
...
The PR exercised the system button, a human tap, consent, and a returned location on an Android 17 emulator, with fallback on API 36. The regular screenshot suite still runs on API 36; broader API 37 coverage is part of the remaining migration work. The [Android and security article](https://www.codenameone.com/blog/android-37-readiness-location-button/) separates those checks and links the current platform guidance.
