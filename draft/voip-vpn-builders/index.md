...
- [Dialogs and secondary windows](https://www.codenameone.com/blog/dialogs-in-native-windows/) now work together, including an opt-in native modal window.
- [The new AppKit port](https://www.codenameone.com/blog/native-appkit-mac-port/) builds a real Mac application instead of presenting an iOS application through Catalyst.
- [SMS one-time-code autofill](https://www.codenameone.com/blog/sms-otp-autofill/) fills a verification code without asking to read the inbox.
- [A daily platform watch](https://www.codenameone.com/blog/platform-deprecation-watch/) now turns Apple and Google notices into source-level checks before deadlines become fires.
- [The new Contact Picker](https://www.codenameone.com/blog/private-contact-picker/) returns selected contact data without broad address-book access.
...
The [window follow-up](https://www.codenameone.com/blog/dialogs-in-native-windows/) covers precedence, fallback behavior, anchored popups, and the remaining limits.
...
The [AppKit article](https://www.codenameone.com/blog/native-appkit-mac-port/) explains the rendering change, build targets, test evidence, and the native accessibility bridge to VoiceOver.
...
Codename One does not send or verify the SMS. Your server still owns expiry, attempt limits, rate limits, and session issuance. The [OTP article](https://www.codenameone.com/blog/sms-otp-autofill/) includes both the complete component and the lower-level text-field option.
...
The task does not make a notice true, and it does not replace platform tests. Its job is to connect a primary-source deadline to the exact builder, generated project, or API that must change. The [platform-watch article](https://www.codenameone.com/blog/platform-deprecation-watch/) shows that chain. Contact selection gets its own [code-focused article](https://www.codenameone.com/blog/private-contact-picker/) because the permission model deserves more than a paragraph.
...
Start with the [Call Management](https://www.codenameone.com/developer-guide/#_call_management) or [VPN](https://www.codenameone.com/developer-guide/#_vpn) chapter, then test the failure path in the simulator before sending a native build.
