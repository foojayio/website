...
First, you need to categorize whether the feature is purely technical. For example, classical rate-limiting to prevent is purely technical. Such technical features belong in the infrastructure: every Reverse-Proxy worth its salt has this kind of rate-limiting.
...
Some libraries offer a centralized configuration solution, *e.g.* , [Spring Cloud Config](https://docs.spring.io/spring-cloud-config/). Carefully evaluate the additional complexity of such a component and its failure rate compared to other dedicated infrastructure components.
...
*Originally published at* [*A Java Geek*](https://blog.frankel.ch/right-feature-right-place/)*on February 19th, 2023*
