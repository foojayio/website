...
<pre class="EnlighterJSRAW" data-enlighter-language="shell">docker run -it --rm apache/apisix:3.4.1-debian \
                 -p 9090:9090 -p 9190:9190 \
                 -v ./config.yaml:/usr/local/apisix/conf/apisix/config.yaml</pre>
...
deployment:
  admin:
    admin_listen:
      port: ${{DEPLOYMENT_ADMIN_ADMIN_LISTEN:=}} #1</pre>
...
<pre class="EnlighterJSRAW" data-enlighter-language="yaml">docker run -it --rm apache/apisix:3.4.1-debian \
...
deployment:
  admin:
    admin_listen:
      port: ${{DEPLOYMENT_ADMIN_ADMIN_LISTEN:=9180}} #1</pre>
