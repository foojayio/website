...
The TLS specification refers to these key exchange schemes as "named groups". Accordingly, the names of these schemes are added to the Named Groups section of the [Java Security Standard Algorithm Names specification](https://docs.oracle.com/en/java/javase/25/docs/specs/security/standard-names.html#named-groups).
...
For more information on this feature, read [JEP 527](https://openjdk.org/jeps/527).
...
Matching is case-insensitive. Filters use [glob patterns](https://en.wikipedia.org/wiki/Glob_(programming)), where `*` and `?` are wildcards. If a filter matches an argument or a key, the argument or the key's value is recorded in the event as `[REDACTED]`.
...
For more information on this feature, see [JEP 536](https://openjdk.org/jeps/536).
...
We can implement this by using a *lazy list*:
...
The API [was changed significantly in Java 26](https://hanno.codes/2026/03/17/java-26-is-here/#whats-different-from-java-25-2), shifting the feature's focus to high-level use cases only.  
...
* A new static `open` method in `StructuredTaskScope` takes a `UnaryOperator` to produce the `StructuredTaskScope` configuration, allowing you to pass a custom configuration while still applying the default join policy.
* The `Joiner` factory methods `allSuccessfulOrThrow()`, `anySuccessfulOrThrow()`, and `awaitAllSuccessfulOrThrow()` now create joiners that cause `join()` to throw an `ExecutionException` when the outcome is an exception. New overloads of the three methods allow a `Function` to be specified to produce a different exception, should you need a different one from the default.
...
* The `PEM` class is now an ordinary class rather than a record. It includes constructors that accept Base64-encoded content in byte arrays, which is more convenient for some use cases.
...
*G1* has been Java's [default garbage collector in server environments since Java 9](https://openjdk.org/jeps/248). Back then, testing showed that the [Serial](https://docs.oracle.com/en/java/javase/24/gctuning/available-collectors.html#GUID-45794DA6-AB96-4856-A96D-FDE5F7DEE498) garbage collector had significant advantages in throughput and footprint in constrained environments with a single CPU or limited available physical memory. And so the decision was made to select Serial as the default garbage collector in those environments.

Since then, the performance of *G1* has improved steadily. With the latest performance improvements ([reduced synchronization](https://hanno.codes/2026/03/17/java-26-is-here/#jep-522-g1-gc-improve-throughput-by-reducing-synchronization)) G1's performance is now sufficient to replace Serial in all situations, including the aforementioned constrained enviroment.
...
* In one setting, the SPECjbb2015 benchmark [used 22% less heap space and 8% less CPU time](https://github.com/rkennke/talks/blob/master/Lilliput-FOSDEM-2025.pdf).
* In another setting, the number of garbage collections done by SPECjbb2015 [was reduced by 15%](https://bugs.openjdk.org/browse/JDK-8350457?focusedId=14766358&page=com.atlassian.jira.plugin.system.issuetabpanels%3Acomment-tabpanel#comment-14766358), with both the G1 and Parallel collectors.
* A highly parallel JSON parser benchmark [ran in 10% less time](https://www.reddit.com/r/scala/comments/1jptiv3/xxusecompactobjectheaders_is_your_new_turbo/?rdt=40432).
