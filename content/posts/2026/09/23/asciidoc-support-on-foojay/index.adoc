...
authors:
  - "frankdelporte"
image: "asciidoc-support-on-foojay.jpg"
categories:
  - "Foojay"
related_posts:
...
this feature, and for naming exactly which parts of it he uses. And of course, https://github.com/foojayio/website/blob/main/content/posts/2026/09/23/asciidoc-support-on-foojay/index.adoc[this post is written in AsciiDoc] to show it off!
...
Code is highlighted by the same highlighter, with the same language names, so `[source,kotlin]` here and a ```` ```kotlin ```` fence in a Markdown article come out looking identical.

And link:/today/diagrams-as-code-mermaid-support-on-foojay/[Mermaid diagrams] work, as a source block like any other:
