...
SMIL animations are supported in the same pipeline: `, `` (``translate``, ``scale``, ``rotate``), and ``. Time values interpolate against wall-clock time on every paint, with ``from`` / ``to`` / ``values`` / ``begin`` / ``dur`` / ``repeatCount`` / ``fill="freeze"` honored.
...
What is still not supported: SVG `filter` primitives, ` (treated as a clip, so alpha masking falls back to opaque), `` (falls back to the first-stop color), and CSS-in-SVG (style rules inside the SVG document; the transcoder reads presentation attributes and the inline ``style="..."`` attribute, but a ` element with selectors is not parsed).
