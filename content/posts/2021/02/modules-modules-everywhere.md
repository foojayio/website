...
This overview is generated by the [Scanner](https://github.com/sormuras/modules/blob/main/src/Scanner.java) program and it is based on the results of the [modulescanner](https://github.com/sandermak/modulescanner) that is run on [Sonatype](https://sonatype.com)s hardware on every uploaded JAR file and stored in an AWS S3 bucket, setup together with the [AdoptOpenJDK](https://adoptopenjdk.net/) team at their [#java9plusadoption](https://adoptopenjdk.slack.com/archives/CB7L6GDUK) Slack channel. Some of those uploaded JAR files are Java modules. They are the interesting subjects of this overview as they contain a `module-info.class`, a compiled module descriptor with a stable name and an explicit API their author(s) comitted to.
...
<pre class="EnlighterJSRAW" data-enlighter-language="generic">Scanning 72598 files in bucket...
  5,204,605 lines
...
<pre class="EnlighterJSRAW" data-enlighter-language="java">String computeMavenGroupAlias(String group) {
  return switch (group) {
