...
Hence, I did some triage on the [APISIX repo](https://github.com/apache/apisix/issues). I stumbled upon this one question:
...
Beyond these three main widespread matching parameters, others are available, *e.g.* , `remote_addrs` or `vars`. Let's focus on the latter. The documentation on the Route API is pretty concise:
...
The browser keeps the cookie, so it's even simpler. Just go to and refresh several times: the content is the same as well. The content will change if you change the cookie to another possible value and request again.
