---
title: "Java Champions"
description: "The Java Champions program recognizes influential members of the Java community. Sourced live from the official Java Champions directory."
url: "/java-champions/"
type: "champions"
# A FEED OF EVERY CHAMPION, newest intake first -- themes/foojay/layouts/champions/single.rss.xml.
# Declared here because hugo.toml's [outputs] takes CLASSES of page
# (home/section/taxonomy/term) and this is a `page`; see the template header for
# why the feed is complete rather than cut to [services.rss] limit.
outputs:
  - html
  - rss
frozen: true
---

The Java Champions program recognizes influential members of the Java community &mdash; presenters, authors, educators, JUG and conference organizers, and others who've helped shape the ecosystem. Read more on [javachampions.org](https://javachampions.org/).
