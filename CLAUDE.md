...
- **`data/jugs.yaml`**: auto-generated by `scripts/FetchJugs.java` — see
  ...
  "jugs"` → `themes/foojay/layouts/jugs/single.html`).
...
2. **None of the jbang scripts have been executed**, including the newest,
   `FetchJugs.java`. The sandbox they were written in blocks outbound
   network access to arbitrary domains (only a markdown-fetch tool was
   available — enough to confirm the GlobalWWJugs frontmatter schema by
   hand, not enough to run the actual GitHub API + raw-file fetch loop), so
   nothing here has been run against the live site, a real Meetup Pro
   account, or the GitHub API. Treat all of it as reviewed-but-untested
   code. **First thing to do for `FetchJugs.java`**: run it locally once
   (`jbang scripts/FetchJugs.java`) and check `data/jugs.yaml` — in
   particular that `founded_date` values came out as plain `yyyy-MM-dd`
   strings (SnakeYAML auto-parses unquoted date-shaped YAML scalars to
   `java.util.Date`, which the script reformats back to a string; if a
   source file's date is formatted unusually this could slip through) and
   that the `/jugs/` table renders sensibly for entries missing most
   optional fields.
